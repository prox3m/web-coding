...
def home(request):
    # 🔑 FIX: Инициализируем форму сразу, чтобы избежать UnboundLocalError
    form = StudentForm()
...
def edit_record(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            grade = int(grade)
            if not (2 <= grade <= 5): errors.append("Оценка должна быть от 2 до 5.")
        except (ValueError, TypeError):
            errors.append("Оценка должна быть целым числом.")
        if errors:
            for err in errors: messages.error(request, f"❌ {err}")
